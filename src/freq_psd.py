...
def flat_by_neighbor_comp(
    mean_rat, cha, st, f_syn_keep, dir_f, n, fac_norm, f_ign, plot_psd_neighbcomp
):
    """
    ...
    diff_next = [abs(mean_rat[i + 1] - mean_rat[i]) for i in range(len(mean_rat[0:-1]))]
    ...
    for i in range(int(num.floor(n / 2)), int(len(sum_diff) - num.floor(n / 2))):
        ...
        if abs(reg[0]) / norm_reg < 1 and num.sqrt(res_lsq / n) < lsq_sqthresh:
            ...
            if f_syn_keep[i - dev] < f_stop or f_syn_keep[i - dev] < f_stop + f_ign:
                f_stop = f_syn_keep[i + dev]
    ...
    if plot_psd_neighbcomp is True:
        ...
        fig.savefig(
            os.path.join(
                dir_f, "%s_%s_%s_flattests.png" % (st.network, st.station, cha)
            )
        )
        plt.close(fig)
        ...
        if i_tup == 0:
            ...
            if tup[0] < current_stop + f_ign:
                ...
                y_flat_f_ranges_sorted.append(
                    num.median(y_flat_f_ranges[i_tup_y_start:i_tup])
                )
                i_tup_y_start = i_tup
...
def get_flat_freqs(n, freqs, rat_a, fac_norm, f_ign, deg=1, plot_flat_range=False):
    """
    ...
    for i in range(dev, int(len(r) - dev)):
        if plot_flat_range is True:
            reg, res_lsq, p, xp = calc_reg_prepplot(
                n, i, f, r, deg, plot=plot_flat_range
            )
            ax4[0].plot(xp, p(xp), "--")
            ax4[0].set_ylim(0, 1 * fac_norm)
        else:
            reg, res_lsq = calc_reg_prepplot(n, i, f, r, deg, plot=plot_flat_range)

        if abs(reg[0]) / fac_norm < 1 and num.sqrt(res_lsq / n) < lsq_sqthresh:
            flat_areas.append(i)

    _flat_f_ranges, y_flat_f_ranges_sorted = get_flat_freq_ranges(
        r, f, n, fac_norm, f_ign, flat_areas
    )
...
def dump_flat_ranges(
    flat_f_ranges_stlist, freq_rat_list_y, nslc_list, dir_f, fname_ext, only_first=True
):
    """
    ...
    flfr = dict_stats(FlatFreqRanges=fr_dict, MeanMedianR_FlatRanges=y_dict)
    flfr.regularize()
    flfr.validate()
...
def const_psd_rat(
    ...
    if plot_flat_range:
        _flat_f_ranges, y_flat_f_ranges_sorted, rmin, rmax, fig4, ax4 = get_flat_freqs(
            n, freqs, rat_a, fac_norm, f_ign, deg=deg, plot_flat_range=plot_flat_range
        )
    ...
    y_flat_f_ranges = [
        y for y, i in zip(y_flat_f_ranges_sorted, _flat_f_ranges) if i[0] != i[1]
    ]
    ...
    if plot_flat_range is True:
        ...
        fig4.savefig(
            os.path.join(
                dir_f,
                "%s_%s_%s_%s_flatrange%s_%s.png" % (net, stat, l, cha, n, fac_norm),
            )
        )
...
def plot_m_ratio(median_rat, f_syn_Z_nonan, nsl, l, dir_f, cha):
    ...
    if not median_rat == []:
        ...
        fig3.tight_layout()
        fig3.savefig(
            os.path.join(dir_f, "%s_%s_%s_%s_mratio.png" % (nsl[0], nsl[1], l, cha))
        )
        plt.close(fig3)
...
def plot_psdratio_from_dict(ratpsd_by_event, st, l, cha, catalog, dir_f):
    ...
    try:
        ...
        fig.savefig(
            os.path.join(
                dir_f, "%s_%s_%s_%s_ratio.png" % (st.network, st.station, l, cha)
            )
        )
    except Exception:
        pass
...
def plot_psd_from_dict(obspsd_by_event, synpsd_by_event, st, l, cha, catalog, dir_f):
    ...
    plt.tight_layout()
    fig.savefig(
        os.path.join(dir_f, "%s_%s_%s_%s.png" % (st.network, st.station, l, cha))
    )
    plt.close(fig)
...
def get_a_f(traces, cha):
    ...
    for tr in traces:
        ...
        a *= tr.deltat * 2.0 / num.sum(win ** 2)  # normalizing
        a[0] /= 2.0  # erster wert durch zwei geteilt.
        a[a.size // 2] /= 2.0  # mittlerer wert durch zwei geteilt.
        a_list.append(a)
...
def calc_plot_psds(
    ...
    for i_ev, ev in enumerate(catalog):
        ...
        trs_obs = data_pile.all(
            ...
            tinc=tinc,
            tpad=tpad,
            trace_selector=lambda tr: tr.nslc_id[:1] == nsl[:1]
            and tr.nslc_id[3] == cha
            and tr.nslc_id[2] == l,
        ...
        if trs_syn and trs_obs:
            ...
            if num.array_equal(f_syn_Z, f_obs_Z) and len(a_syn_Z) > 0:
                ...
                ratio_a = [
                    a1 / a2 if not a2 == 0 else 0 for a1, a2 in zip(a_syn_Z, a_obs_Z)
                ]
    ...
    return obspsd_by_event, synpsd_by_event, ratpsd_by_event, f_syn_keep, ratio_npar
...
def prep_psd_fct(
    ...
    st_data_pile = pile.make_pile(
        datapath, regex="%s_%s_" % (st.network, st_name), show_progress=False
    )
    st_syn_data_pile = pile.make_pile(
        syndatapath, regex="%s_%s_" % (st.network, st_name), show_progress=False
    )
    freq_rat_list = []
    freq_rat_list_y = []
    nslc_list = []
    ...
    if (
        st_data_pile.tmin is not None
        and st_data_pile.tmax is not None
        and st_syn_data_pile.tmin is not None
        ...
        for cha in ["Z", "R", "T"]:
            ...
            if plot_psds is True and outs[0] and outs[1]:
                plot_psd_from_dict(outs[0], outs[1], st, l, cha, subset_catalog, dir_f)

                if plot_ratio_extra is True and outs[2]:
                    plot_psdratio_from_dict(outs[2], st, l, cha, subset_catalog, dir_f)
