...
deg = 1
fac_norm = 50  # rmin/df
f_ign = 0.02  # igonore gaps of f_ign [Hz] in flat areas due to outlayers!


def flat_by_neighbor_comp(
    mean_rat, cha, st, f_syn_keep, dir_f, plot_psd_neighbcomp):
    '''
...
def get_flat_freqs(n, freqs, rat_a, deg=1, plot_flat_range=False):
    ...
    if plot_flat_range is True:
        ...
        ax4[0].set_ylabel('mean PSD ratio (syn/obs)')
    ...
    _flat_f_ranges, y_flat_f_ranges_sorted =\
        get_flat_freq_ranges(r, f, n, flat_areas)
...
def const_psd_rat(mean_rat, cha, st, f_syn_keep,
    ...
    if plot_flat_range:
        ...
        get_flat_freqs(n, freqs, rat_a, deg=deg,
...
def plot_m_ratio(mean_rat, f_syn_Z_nonan, nsl, dir_f, cha):
    '''
    ...
    if not mean_rat == []:
        ...
        ax3.set_ylabel('mean PSD ratio (syn/obs)')
...
def plot_psdratio_from_dict(ratpsd_by_event, st, cha, catalog, dir_f):
    ...
    for i_ev, ev in enumerate(catalog):
        ...
        ax[i_x, i_y].set_ylim(r_min, r_max)
...
def plot_psd_from_dict(obspsd_by_event, synpsd_by_event, st, cha, catalog, dir_f):
...
def calc_plot_psds(catalog, data_pile, syn_data_pile,
    ...
    for cha in ['Z', 'R', 'T']:
        if plot_psds is True:
            # fig, ax = plt.subplots(nrows=nrows, ncols=ncols, figsize=(13, 9))
            # nplots = 0
            # dict for later plotting all events of nslc at once
            obspsd_by_event = {}
            synpsd_by_event = {}
        ...
        if plot_psds is True:
            plot_psd_from_dict(obspsd_by_event, synpsd_by_event, st, cha, catalog, dir_f)

        if plot_ratio_extra is True:
        	plot_psdratio_from_dict(ratpsd_by_event, st, cha, catalog, dir_f)

        if ratio_npar.shape != (1,):
            m_rat = calc_median_ratio(ratio_npar)
            # calc_mean_ratio(ratio_npar)
...
def prep_psd_fct(i_st, st, subset_catalog, dir_f, arrT_array, datapath,
    ...
    if st_data_pile.tmin is not None and st_data_pile.tmax is not None and\
        ...
        freq_rat_list, freq_rat_list_y, nslc_list,\
            flat_by_next, flat_by_next_y =\
            calc_plot_psds(
                subset_catalog,
                st_data_pile,
