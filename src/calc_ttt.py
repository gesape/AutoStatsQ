...
'''
File containing functions to compute travel time tables for events/stations.
'''
...
def get_ttt(ct, coords, depth_opt):
    x_bounds = num.array(
        [[ct.r_depth_min, ct.r_depth_max],
         ...
         [ct.dist_min, ct.dist_max]], dtype=num.float)
    ...
    earthmodel = CakeEarthmodel(id=ct.earthmodel_id,
                                ...
                                cake.builtin_model_filename(ct.earthmodel_id)))

    interpolated_tts = {}

    for phase_def in ct.tabulated_phases:

        ttt_hash_ = ttt_hash(ct,
                             earthmodel, phase_def.phases,
                             x_bounds, x_tolerance,
                             t_tolerance)
