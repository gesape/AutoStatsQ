...
def get_pl_opt(lats_all, lons_all):
    ...
    corners = [
        od.Loc(num.min(lats_all), num.min(lons_all)),
...
def main():
    ...
    parser.add_argument("--logoutput", "-o", default=None, help="File to save the log")
    ...
    if args.generate_config:
        logging.info(
            "Welcome to AutoStatsQ - a station quality control checking tool.\n"
        )
    ...
    if args.run:
        ...
        for stat_list in gensettings.list_station_lists:
            if stat_list.endswith(".csv"):
                with open(stat_list, "r") as f:
                    for line in f.readlines():
                        if len(line.strip().split(",")) == 6:
                            ...
                            n_s = "%s.%s" % (n, s)
                            if (
                                n_s in gensettings.st_use_list
                                or gensettings.st_use_list == []
                            ):
                                all_stations.append(
                                    model.Station(
                                        network=n,
                ...
                msg = (
                    "Station file extension not known: %s. Please use .xml, .csv or .yaml."
                    % stat_list
                )
                logs.error(msg)
        ...
        if catalogconf.search_events is True:
            ...
            event_names = geofon.get_event_names(
                time_range=(tmin, tmax), magmin=catalogconf.min_mag
            )
            ...
            catfilename = os.path.join(
                data_dir, "results/catalog", "catalog_Mgr%s.txt" % (catalogconf.min_mag)
            )
        ...
        if catalogconf.use_local_catalog is True:
            ...
            if not hasattr(catalogconf, "catalog_fn") or catalogconf.catalog_fn is None:
                catalogconf.catalog_fn = os.path.join(
                    data_dir,
                    "results/catalog",
            ...
            if catalogconf.subset_of_local_catalog is True:
                if not catalogconf.mid_point:
                    ...
                    mid_point = orthodrome.geographic_midpoint(
                        num.asarray(st_lats), num.asarray(st_lons)
                    )

                else:
                    mid_point = catalogconf.mid_point

                ev_catalog = subset_events_dist_cat(
                    catalogconf.catalog_fn,
                    catalogconf.min_mag,
                    catalogconf.max_mag,
        ...
        for d, val in catalogconf.depth_options.items():
            if not catalogconf.use_local_subsets:
                ev_cat = subset_events_dist_evlist(
                    ev_catalog,
                    catalogconf.min_mag,
                    catalogconf.max_mag,
                    catalogconf.tmin_str,
                ...
                for i_ev, ev in enumerate(ev_cat):
                    ...
                    bazi_array[i_ev, :] = [
                        orthodrome.azibazi(ev.lat, ev.lon, lat, lon)[1]
                        for (lat, lon) in zip(st_lats, st_lons)
                    ]

                    bazi_mp_array[i_ev] = orthodrome.azibazi(
                        ev.lat, ev.lon, mid_point[0], mid_point[1]
                    )[1]
                ...
                mean_wedges_mp = num.where(
                    wedges_array_mp < 0, no_bins + wedges_array_mp, wedges_array_mp
                )
                ...
                if catalogconf.plot_dist_vs_magn is True:
                    logs.info(
                        " Plotting catalog distance vs. magnitude for full catalog: %s."
                        % d
                    )
                ...
                for bin_nr in range(no_bins):
                    ...
                    if len(bin_ev_ind) > 1:
                        ...
                        if (
                            bin_nr != 0
                            and bin_nr != no_bins - 1
                            and hist[bin_nr - 1] == 0
                            and hist[bin_nr + 1] == 0
                        ):
                            # choose min und max bazi for better azimuthal coverage
                            min_bazi_ev_ind = bin_ev_ind[
                                num.argmin(bazi_mp_array[bin_ev_ind])
                            ][0]
                            max_bazi_ev_ind = bin_ev_ind[
                                num.argmax(bazi_mp_array[bin_ev_ind])
                            ][0]
                            ...
                            ev_ind_next = bin_ev_ind[
                                num.argsort(bazi_mp_array[bin_ev_ind], axis=0)[0]
                            ][0][0]
                            ...
                            ev_ind_next = bin_ev_ind[
                                num.argsort(bazi_mp_array[bin_ev_ind], axis=0)[
                                    len(bin_ev_ind) - 1
                                ]
                            ][0][0]
                            subset_catalog.append(ev_cat[ev_ind_next])
                ...
                logs.info(
                    "Subset of %d events was generated for %s."
                ...
                try:
                    if not hasattr(catalogconf, "subset_fns") or (
                        d not in catalogconf.subset_fns
                    ):
                        catalogconf.subset_fns[d] = os.path.join(
                            data_dir,
                            "results/catalog",
                ...
                logs.info(
                    " Subset of catalog for %s: %s events.\n" % (d, len(subset_catalog))
                )
            ...
            if catalogconf.plot_hist_wedges is True:
                ...
                for i_ev, ev in enumerate(subset_catalog):
                    ...
                    bazi_array_subset[i_ev, :] = [
                        orthodrome.azibazi(ev.lat, ev.lon, lat, lon)[1]
                        for (lat, lon) in zip(st_lats, st_lons)
                    ]

                    bazi_mp_array_subset[i_ev] = orthodrome.azibazi(
                        ev.lat, ev.lon, mid_point[0], mid_point[1]
                    )[1]

                wedges_array_subset = num.floor(
                    bazi_array_subset / catalogconf.wedges_width
                )

                # hist based on chosen mid_point of array
                wedges_array_mp_subset = num.floor(
                    bazi_mp_array_subset / catalogconf.wedges_width
                )
                mean_wedges_mp_subset = num.where(
                    wedges_array_mp_subset < 0,
                    no_bins + wedges_array_mp_subset,
                    wedges_array_mp_subset,
                )
                bins_hist_subset = [a for a in range(no_bins + 1)]
                hist_subset, bin_edges_subset = num.histogram(
                    mean_wedges_mp_subset, bins=bins_hist_subset
                )
            ...
            if catalogconf.plot_dist_vs_magn is True:
                ...
                logs.info(
                    " Plotting catalog distance vs. magnitude for subset: %s." % d
                )
                plot_distmagn(
                    dist_array_subset, subset_catalog, data_dir, d, full_cat=False
                )
            ...
            if arrTconf.calc_first_arr_t is True:
                logs.info(" Calculating arrival times of first phase, %s subset." % d)
                data_dir = gensettings.work_dir
                ...
                phases = [
                    cake.PhaseDef(pid) for pid in arrTconf.phase_select.split("|")
                ]
                ...
                for i_ev, ev in enumerate(subset_catalog):
                    ...
                    for i_st in range(len(ns)):
                        ...
                        arrivals = vmodel.arrivals(
                            distances=[dist * cake.m2d], phases=phases, zstart=ds
                        )
                ...
                num.save(
                    os.path.join(data_dir, "ttt", "ArrivalTimes_%s" % (d)), arrT_array
                )
            ...
            if arrTconf.calc_est_R is True:
                logs.info(
                    " Calculating arrival times of Rayleigh waves, %s subset." % d
                )
                data_dir = gensettings.work_dir
                os.makedirs(os.path.join(data_dir, "ttt"), exist_ok=True)
                dist_array_sub = num.empty((len(subset_catalog), len(ns)))
                ...
                for i_ev, ev in enumerate(subset_catalog):
                    for i_st in range(len(ns)):
                        ...
                        arrT_R_array[i_ev, i_st] = ev.time + dist / (
                            arrTconf.v_rayleigh * 1000.0
                        )

                num.save(
                    os.path.join(data_dir, "ttt", "ArrivalTimes_estR_%s" % (d)),
                    arrT_R_array,
                )
        ...
        if metaDataconf.download_data is True:  ### clean up!
            ...
            for subset_catalog in subsets_events.values():
                for ev in subset_catalog:
                    ...
                    t_start = util.str_to_time(
                        str(ev_t - dt_s), format="%Y-%m-%d %H:%M:%S.OPTFRAC"
                    )
                    t_end = util.str_to_time(
                        str(ev_t + dt_e), format="%Y-%m-%d %H:%M:%S.OPTFRAC"
                    )
                    ...
                    for ns_now in ns:
                        mseed_fn_st = os.path.join(
                            dir_make, "%s_%s_%s" % (ns_now[0], ns_now[1], ev_t_str)
                        )
                        ...
                        for site in sites:
                            ...
                            try:
                                if token:
                                    request_waveform = fdsn.dataselect(
                                        site=site, selection=selection, token=token
                                    )
                                else:
                                    request_waveform = fdsn.dataselect(
                                ...
                                logs.debug(
                                    "%s data downloaded from %s" % (ns_now, site)
                                )
                                break
        ...
        if metaDataconf.download_metadata is True:
            ...
            cat_tmin = min(
                [
                    ev.time
                    for ev in subset_catalog
                    for k, subset_catalog in subsets_events.items()
                ]
            )
            cat_tmax = max(
                [
        ...
        if RestDownconf.rest_data is True:
            ...
            if metaDataconf.use_downmeta is True:
                ...
                for site in sites:
                    stations_fn = os.path.join(
                        data_dir, "Resp_all_" + str(site) + ".xml"
                    )
            ...
            if not metaDataconf.use_downmeta and not metaDataconf.local_metadata:
                logs.error(
                    " No response files found. Set *use_downmeta* to True "
            ...
            if metaDataconf.local_data and not metaDataconf.sds_structure:
                ...
                p_local = pile.make_pile(
                    paths=metaDataconf.local_data, show_progress=True
                )
            ...
            for key, subset_catalog in subsets_events.items():
                ...
                for i_ev, ev in enumerate(subset_catalog):
                    ...
                    if metaDataconf.local_waveforms_only is False:
                        p = pile.make_pile(
                            paths=os.path.join(data_dir, ev_t_str), show_progress=False
                        )
                    ...
                    for i_st, st in enumerate(all_stations):
                        print(
                            "Event: %5d/%s, Station: %5d/%s" % (i_ev, nev, i_st, nst),
                            end="\r",
                        )
                            ...
                            if metaDataconf.sds_structure is False:
                                ...
                                for i_ldd, ldd in enumerate(local_data_dirs):
                                    path_str = os.path.join(
                                        ldd, year, st.network, st.station
                                    )
                                    p = pile.make_pile(
                                        paths=path_str,
                                        regex=".%s" % jul_day,
                            ...
                            if metaDataconf.all_channels == False:
                                ...
                                if "HHZ" in comps and "HHN" in comps and "HHE" in comps:
                                    trs = [
                                        tr
                                        for tr in trs
                                        if tr.channel in ["HHZ", "HHN", "HHE"]
                                    ]
                                elif (
                                    "HHZ" in comps and "HH2" in comps and "HH3" in comps
                                ):
                                    trs = [
                                        tr
                                        for tr in trs
                                        if tr.channel in ["HHZ", "HH2", "HH3"]
                                    ]
                                elif (
                                    "HHZ" in comps and "HH1" in comps and "HH1" in comps
                                ):
                                    trs = [
                                        tr
                                        for tr in trs
                                        if tr.channel in ["HHZ", "HH1", "HH2"]
                                    ]

                                elif (
                                    "BHZ" in comps and "BHN" in comps and "BHE" in comps
                                ):
                                    trs = [
                                        tr
                                        for tr in trs
                                        if tr.channel in ["BHZ", "BHN", "BHE"]
                                    ]
                                elif (
                                    "BHZ" in comps and "BH2" in comps and "BH3" in comps
                                ):
                                    trs = [
                                        tr
                                        for tr in trs
                                        if tr.channel in ["BHZ", "BH2", "BH3"]
                                    ]
                                elif (
                                    "BHZ" in comps and "BH1" in comps and "BH2" in comps
                                ):
                                    trs = [
                                        tr
                                        for tr in trs
                                        if tr.channel in ["BHZ", "BH1", "BH2"]
                                    ]

                                elif (
                                    "HNZ" in comps and "HNN" in comps and "HNE" in comps
                                ):
                                    trs = [
                                        tr
                                        for tr in trs
                                        if tr.channel in ["HNZ", "HNN", "HNE"]
                                    ]
                                elif (
                                    "HNZ" in comps and "HN2" in comps and "HN3" in comps
                                ):
                                    trs = [
                                        tr
                                        for tr in trs
                                        if tr.channel in ["HNZ", "HN2", "HN3"]
                                    ]
                                elif (
                                    "HNZ" in comps and "HN1" in comps and "HN2" in comps
                                ):
                                    trs = [
                                        tr
                                        for tr in trs
                                        if tr.channel in ["HNZ", "HN1", "HN2"]
                                    ]

                                elif (
                                    "EHZ" in comps and "EHN" in comps and "EHE" in comps
                                ):
                                    trs = [
                                        tr
                                        for tr in trs
                                        if tr.channel in ["EHZ", "EHN", "EHE"]
                                    ]
                                elif (
                                    "EHZ" in comps and "EH2" in comps and "EH3" in comps
                                ):
                                    trs = [
                                        tr
                                        for tr in trs
                                        if tr.channel in ["EHZ", "EH2", "EH3"]
                                    ]
                                elif (
                                    "EHZ" in comps and "EH1" in comps and "EH2" in comps
                                ):
                                    trs = [
                                        tr
                                        for tr in trs
                                        if tr.channel in ["EHZ", "EH1", "EH2"]
                                    ]

                                elif (
                                    "LHZ" in comps and "LHN" in comps and "LHE" in comps
                                ):
                                    trs = [
                                        tr
                                        for tr in trs
                                        if tr.channel in ["LHZ", "LHN", "LHE"]
                                    ]
                                elif (
                                    "LHZ" in comps and "LH2" in comps and "LH3" in comps
                                ):
                                    trs = [
                                        tr
                                        for tr in trs
                                        if tr.channel in ["LHZ", "LH2", "LH3"]
                                    ]
                                elif (
                                    "LHZ" in comps and "LH1" in comps and "LH2" in comps
                                ):
                                    trs = [
                                        tr
                                        for tr in trs
                                        if tr.channel in ["LHZ", "LH1", "LH2"]
                                    ]

                                elif (
                                    "CNZ" in comps and "CNN" in comps and "CNE" in comps
                                ):
                                    trs = [
                                        tr
                                        for tr in trs
                                        if tr.channel in ["CNZ", "CNN", "CNE"]
                                    ]
                                elif (
                                    "CNZ" in comps and "CN2" in comps and "HH3" in comps
                                ):
                                    trs = [
                                        tr
                                        for tr in trs
                                        if tr.channel in ["CNZ", "CN2", "CN3"]
                                    ]
                                elif (
                                    "CNZ" in comps and "CN1" in comps and "CN2" in comps
                                ):
                                    trs = [
                                        tr
                                        for tr in trs
                                        if tr.channel in ["CNZ", "CN1", "CN2"]
                                    ]

                                elif (
                                    "CHZ" in comps and "CHN" in comps and "CHE" in comps
                                ):
                                    trs = [
                                        tr
                                        for tr in trs
                                        if tr.channel in ["CHZ", "CHN", "CHE"]
                                    ]
                                elif (
                                    "CHZ" in comps and "CH2" in comps and "CH3" in comps
                                ):
                                    trs = [
                                        tr
                                        for tr in trs
                                        if tr.channel in ["CHZ", "CH2", "CH3"]
                                    ]
                                elif (
                                    "CHZ" in comps and "CH1" in comps and "CH2" in comps
                                ):
                                    trs = [
                                        tr
                                        for tr in trs
                                        if tr.channel in ["CHZ", "CH1", "CH2"]
                                    ]

                                elif (
                                    "DNZ" in comps and "DNN" in comps and "DNE" in comps
                                ):
                                    trs = [
                                        tr
                                        for tr in trs
                                        if tr.channel in ["DNZ", "DNN", "DNE"]
                                    ]
                                elif (
                                    "DNZ" in comps and "DN2" in comps and "DN3" in comps
                                ):
                                    trs = [
                                        tr
                                        for tr in trs
                                        if tr.channel in ["DNZ", "DN2", "DN3"]
                                    ]
                                elif (
                                    "DNZ" in comps and "DN1" in comps and "DN2" in comps
                                ):
                                    trs = [
                                        tr
                                        for tr in trs
                                        if tr.channel in ["DNZ", "DN1", "DN2"]
                                    ]
                                elif (
                                    "DN1" in comps and "DN2" in comps and "DN3" in comps
                                ):
                                    trs = [
                                        tr
                                        for tr in trs
                                        if tr.channel in ["DN1", "DN2", "DN3"]
                                    ]

                                elif (
                                    "DHZ" in comps and "DHN" in comps and "DHE" in comps
                                ):
                                    trs = [
                                        tr
                                        for tr in trs
                                        if tr.channel in ["DHZ", "DHN", "DHE"]
                                    ]
                                elif (
                                    "DHZ" in comps and "DH2" in comps and "DH3" in comps
                                ):
                                    trs = [
                                        tr
                                        for tr in trs
                                        if tr.channel in ["DHZ", "DH2", "DH3"]
                                    ]
                                elif (
                                    "DHZ" in comps and "DH1" in comps and "DH2" in comps
                                ):
                                    trs = [
                                        tr
                                        for tr in trs
                                        if tr.channel in ["DHZ", "DH1", "DH2"]
                                    ]
                                elif (
                                    "DH1" in comps and "DH2" in comps and "DH3" in comps
                                ):
                                    trs = [
                                        tr
                                        for tr in trs
                                        if tr.channel in ["DH1", "DH2", "DH3"]
                                    ]
                                ...
                                for tr in trs:
                                    ...
                                    for resp_now in responses:
                                        try:
                                            ...
                                            if cnt_resp == i_resp:
                                                logs.warning(
                                                    "no resp found: %s" % (tr.nslc_id,)
                                                )

                                        except trace.TraceTooShort:
                                            logs.error(
                                                "trace too short: %s" % (tr.nslc_id,)
                                            )

                                        except ValueError:
                                            logs.error(
                                                "downsampling does not work: %s"
                                                % (tr.nslc_id,)
                                            )
            ...
            logs.info(
                " Saved restituted data in directory %s.\n"
        ...
        if RestDownconf.rotate_data is True:
            ...
            def downsample_rotate(dir_rest, dir_rot, all_stations, st_xml, deltat_down):
                ...
                ev_data_pile = pile.make_pile(
                    dir_rest, regex="rest2", show_progress=False
                )
                    ...
                    for l in loc_avail:
                        for ch_ in ch_list:
                            lens_trs = [
                                len(tr.ydata)
                                for tr in trs
                                if tr.channel[0:2] == ch_ and tr.location == l
                            ]
                            trs_ch = [
                                tr
                                for tr in trs
                            ...
                            if not lens_trs or 0 in lens_trs or len(lens_trs) != 3:
                                ...
                                for st_now in st_xml:
                                    stat = st_now.get_pyrocko_stations(
                                        nslcs=nslcs, timespan=(trmin, trmax)
                                    )
                                    if len(stat) == 1:
                                        break
                                ...
                                if len(stat) != 0:
                                    ...
                                    if "1" in test and "2" in test and "3" not in test:
                                        ...
                                        "2" in test and "3" in test and "1" not in test
                                    ):
                                        naming = "2,3"
                                    ...
                                    for tr in trs_ch:
                                        if (
                                            tr.channel.endswith("N")
                                            or (
                                                tr.channel.endswith("2")
                                                and naming == "2,3"
                                            )
                                            or (
                                                tr.channel.endswith("1")
                                                and naming == "1,2"
                                            )
                                            or (
                                                tr.channel.endswith("2")
                                            ...
                                            for ch in stat.channels:
                                                if tr.channel == ch.name:
                                                    ...
                                                    try:
                                                        ...
                                                        tr1 = None
                                                        logs.error(
                                                            "N/2 downsampling not successful"
                                                        )
                                                    except util.UnavailableDecimation:
                                                        logs.error(
                                                            "unavailable decimation %s"
                                                            % tr1.station
                                                        )
                                                        tr1 = None

                                        if (
                                            tr.channel.endswith("E")
                                            or (
                                                tr.channel.endswith("3")
                                                and naming == "2,3"
                                            )
                                            or (
                                                tr.channel.endswith("2")
                                            ...
                                            for ch in stat.channels:
                                                if tr.channel == ch.name:
                                                    ...
                                                    try:
                                                        ...
                                                        tr2 = None
                                                        logs.error(
                                                            "E/3 downsampling not successful"
                                                        )
                                                    except util.UnavailableDecimation:
                                                        logs.error(
                                                            "unavailable decimation %s"
                                        ...
                                        if (
                                            ...
                                            try:
                                                ...
                                                logs.error(
                                                    "E/3 comp no data in twd %s" % nsl
                                                )
                                            except ValueError:
                                                trZ = None
                                                logs.error(
                                                    "Z downsampling not successful"
                                                )
                                            except util.UnavailableDecimation:
                                                logs.error(
                                                    "unavailable decimation %s"
                                ...
                                if (
                                    ...
                                    if (
                                        str(tr1.channel).endswith("N") is True
                                        or str(tr1.channel).endswith("2") is True
                                        and naming == "2,3"
                                        or str(tr1.channel).endswith("1") is True
                                        and naming == "1,2"
                                        or str(tr1.channel).endswith("2") is True
                                        and naming == "1,2,3"
                                    ):
                                    ...
                                    if (
                                        str(tr1.channel).endswith("E") is True
                                        or str(tr1.channel).endswith("3") is True
                                        and naming == "2,3"
                                        or str(tr1.channel).endswith("2") is True
                                        and naming == "1,2"
                                        or str(tr1.channel).endswith("1") is True
                                    ...
                                    if (
                                        str(tr2.channel).endswith("N") is True
                                        or str(tr2.channel).endswith("2") is True
                                        and naming == "2,3"
                                        or str(tr2.channel).endswith("1") is True
                                        and naming == "1,2"
                                        or str(tr2.channel).endswith("2") is True
                                        and naming == "1,2,3"
                                    ):
                                    ...
                                    if (
                                        str(tr2.channel).endswith("E") is True
                                        or str(tr2.channel).endswith("3") is True
                                        and naming == "2,3"
                                        or str(tr2.channel).endswith("2") is True
                                        and naming == "1,2"
                                        or str(tr2.channel).endswith("1") is True
                                    ...
                                    if tr1_ch != "0" and tr2_ch != "0":
                                        ...
                                        tracestouse = [
                                            ...
                                            if num.any(num.diff(tr.ydata))
                                        ]
                                        lens_trsuse = [
                                            len(tr.ydata) for tr in tracestouse
                                        ]
                                        if (
                                            not lens_trsuse
                                            or 0 in lens_trsuse
                                            or len(lens_trsuse) != 2
                                        ):
                                            continue
            ...
            logs.info(
                " Done with rotation to ZRT. Saved rotated and"
        ...
        if synthsconf.make_syn_data is True:
            ...
            if not os.path.exists(synthsconf.engine_path):
                logs.error(
                    "ERROR: Engine path not found: %s." % synthsconf.engine_path
                    + " Please add a valid path to *engine_path* to compute synthetic data."
                )
                sys.exit()

            if not os.path.exists(
                os.path.join(synthsconf.engine_path, synthsconf.store_id)
            ):
                logs.error(
                    "ERROR: GF store not found: %s."
                    % os.path.join(synthsconf.engine_path, synthsconf.store_id)
                )
                sys.exit()
            ...
            engine = gf.LocalEngine(store_superdirs=[synthsconf.engine_path])
            ...
            for key, subset_catalog in subsets_events.items():
                ...
                for i_ev, ev in enumerate(subset_catalog):
                    ...
                    for ist, st in enumerate(all_stations):
                        print(
                            "Event: %5d/%s, Station %5d/%s" % (i_ev, nev, ist, nst),
                            end="\r",
                        )
                        targets = []
                        sta = st.station
                        ...
                        for cha in ["Z", "R", "T"]:
                            ...
                            if cha == "R":
                                target.azimuth = azi - 180.0
                                target.dip = 0.0

                            elif cha == "T":
                                target.azimuth = azi - 90.0
                                target.dip = 0.0
            ...
            logs.info(
                " Done with computation of synth. waveforms."
        ...
        if gainfconf.calc_gainfactors is True:
            ...
            def run_autogain(
                ...
                if gain_factor_method == "syn_comp":
                    ...
                    if syn_data_pile.is_empty():
                        logs.error(
                            " No synthetic data found in %s. Gain test failed."
                            % os.path.join(data_dir, "synthetics")
                        )
                        sys.exit()
                ...
                logs.debug(
                    " Saving mean gains: gains_median_and_mean%s.txt %s"
                    % (c, dir_gains)
                )
                ag.save_median_and_mean_and_stdev(
                    "gains_median_and_mean%s.txt" % c, directory=dir_gains
                )
                # Store all results in comma-spread text file:
                ag.save_single_events(
                    "gains_all_events%s.txt" % c,
                    directory=dir_gains,
                    plot=gainfconf.plot_allgains,
                )
                ag = None
            ...
            gc.collect()
            logs.info(
                " Finished evaluation of gain factors."
        ...
        if gainfconf.plot_median_gain_on_map is True:
            ...
            if maps.pl_opt == ["automatic"]:
                ...
                for o in maps.pl_opt[0:2]:
                    if isinstance(o, str):
                        logs.warning(
                            " Please make sure the %s in the map plotting option in the confic files is set."
                            % o
                        )
                        skip_plot = True
                if not skip_plot:
                ...
                logs.warning(
                    " Set pl_opt to *automatic* or provide [lat, lon, radius, cscale]."
                )
                skip_plot = True
            ...
            if not skip_plot:
                for c in gainfconf.components:
                    ...
                    plot_median_gain_map_from_file(
                        ns,
                        st_lats,
                        st_lons,
                        pl_opt,
                        maps.pl_topo,
                        "gains_median_and_mean%s.txt" % c,
                        dir_gains,
                        c,
                        maps.map_size,
                        maps.outformat,
        ...
        if psdsconf.calc_psd is True:
            ...
            if arrT_R_array is None:
                try:
                    data_dir = gensettings.work_dir
                    atrfile = os.path.join(
                        data_dir, "ttt", "ArrivalTimes_estR_deep.npy"
                    )
        ...
        if orientconf.plot_orient_map_fromfile is True:
            ...
            if not skip_plot:
                orient.plot_corr_angles(
                    ns,
                    st_lats,
                    st_lons,
                    "CorrectionAngles.yaml",
                    dir_ro,
                    pl_opt,
                    maps.pl_topo,
                    maps.map_size,
        ...
        if timingconf.timing_test is True:
            ...
            if arrT_array is None:
                try:
                    data_dir = gensettings.work_dir
                    arrT_array = num.load(
                        os.path.join(data_dir, "ttt", "ArrivalTimes_deep.npy")
                    )
                except:
                    logs.error("Please calculate arrival times first!")
            ...
            for i_ev, ev in enumerate(subset_catalog):
                tshifts[:, i_ev] = tt.ccs_allstats_one_event(
                    i_ev,
                    nev,
                    ev,
                    stations,
        ...
        if tc.tele_check is True:
            ...
            for ev in subset_catalog:
                ...
                p_obs = pile.make_pile(
                    os.path.join(datapath, ev.name), show_progress=False
                )
                if p_obs.is_empty():
                    continue
                p_obs.snuffle(
                    stations=all_stations, events=[ev], launch_hook=load_snuffling
                )
