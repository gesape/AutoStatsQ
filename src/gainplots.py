...
def plot_allgains(self, results_all, stats_list, directory, fn):
    ...
    n_plotrows = math.ceil(n_stats / 50.0) + 1
    ...
    for i, (row, axes) in enumerate(zip(range(n_plotrows), list(ax))):
        ...
        for i_st in range(start, stop):
            ...
            if len(self.method) == 2:
                axes.set_ylabel(
                    "Gain relative to %s %s"
                    % (str(self.method[1][0]), str(self.method[1][1])),
                    fontsize=14,
                )
            ...
            stats = [
                str(st[0]) + " " + str(st[1]) for st in list(stats_list)[start:stop]
            ]
            if self.method == "syn":
...
def plot_median_gain_map_from_file(
    ns,
    st_lats,
    st_lons,
    pl_options,
    ...
    for i_ns, ns_now in enumerate(ns):
        for l in ["00", "", "01", "10", "60"]:
            try:
                ...
                if (
                    g > 0.0 or g < 0.0
                ):  # g < 5 and g > 0.2:  # g < 10.0 and g > 0.1: #g > 0.0 or g < 0.0:
    ...
    m.gmt.makecpt(
        C=pl_options[
            3
        ],  #'split',#'polar',  # '/home/gesap/Documents/CETperceptual_GMT/CET-D4.cpt',#'split',#'polar',
    ...
    m.gmt.psxy(
        in_columns=(lon_no_nan, lat_no_nan, gains_no_nan),
        C=cptfile,
        G="blue",
        S="t24p",
        *m.jxyr
    ...
    fn = os.path.join(
        directory, "%s%s_map_log.%s" % (gains_file[0:12], comp, outformat)
    )
    m.save(fn)
