...
def ccs_allstats_one_event(
    ...
    if p_obs and p_syn:
        for i_st, st in enumerate(stat_list):
            print("Event: %5d/%s, Station: %5d/%s" % (i_ev, nev, i_st, nst), end="\r")

            try:
                logs.debug("Checking timing, station: %s.%s" % (st.network, st.station))
                s = st.station
                n = st.network
                l = "not_set"
            except:
                n, s, l, c = st
                logs.debug("Checking timing, station: %s.%s" % (n, s))

            i_ast = [
                i_ast
                for i_ast, ast in enumerate(all_stations)
            ...
            if l != "not_set":
                tr_obs = p_obs.all(
                    trace_selector=lambda tr: tr.network == n
                    and tr.station == s
                    and tr.location == l
                    and tr.channel == "Z",
                    tmin=tmin,
...
def plot_matrix(tshifts, tshifts_cor, stations, dir_time):
    ...
    a = ax[0].imshow(tshifts, vmin=-absmax, vmax=+absmax, interpolation="nearest")
    ...
    b = ax[1].imshow(tshifts_cor, vmin=-absmax, vmax=absmax, interpolation="nearest")
...
def plot_tshifts(tshifts_cor, means, stdevs, outfile, stations):
    ...
    try:
        stations_sorted = sorted(
            stations, key=lambda k: "%s.%s" % (k.network, k.station)
        )
        indices_st = [
            i
            for (i, j) in sorted(
        ...
        stations_sorted = sorted(
            stations, key=lambda k: "%s.%s.%s" % (k[0], k[1], k[2])
        )
        indices_st = [
            i
            for (i, j) in sorted(
    ...
    cnt = 0
    fig, ax = plt.subplots(
        nrows=n_plotrows, figsize=(10, n_plotrows * 3), squeeze=False
    )
    ...
    for i_row in range(n_plotrows):
        ...
        try:
            stats = [
                "%s.%s" % (st.network, st.station)
                for st in stations_sorted[
                    i_row
                    + (i_row * (n_per_row - 1)) : (n_per_row - 1) * (i_row + 1)
                    + 1
                ]
            ]
        except AttributeError:
...
def save_mms(medians, means, stdevs, stations, out_dir, n_evs):
    ...
    try:
        ...
        meds = dict(
            zip(
                ["%s.%s.%s" % (s.network, s.station, s.location) for s in stations],
                medians,
            )
        )
        ms = dict(
