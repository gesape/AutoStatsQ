...
def gen_report(config):
    ...
    try:
        os.makedirs(os.path.join(gensettings.work_dir,
                                 'result_report'))
    except:
        rmtree(os.path.join(gensettings.work_dir,
                            'result_report'))
        os.makedirs(os.path.join(gensettings.work_dir,
                                 'result_report'))
    ...
    for h in html_directories:
        copytree(src=os.path.join(file_path,
                                  'generate_report', h),
                 dst=os.path.join(os.path.join(
                                  gensettings.work_dir,
                                  'result_report'), h))

    outfile = os.path.join(gensettings.work_dir,
                           'result_report',
                           'index_report.html')
    ...
    with open(tpl_file, 'r') as f:
        ...
        for line in lines:
            if "INCLUDE_HERE_CAT" in line:
                ...
                extended_lines.append(cat.include_maps_subsets(
                                      tmin, tmax, cat_result_dir))
                extended_lines.append(cat.include_event_tabels(
                                      tmin, tmax, Mmin, cat_result_dir))
                extended_lines.append(cat.include_statistic_plots(
                                      cat_result_dir))
                ...
                extended_lines.append(ori.include_map(orient_result_dir,
                    orientconf.ccmin))
                extended_lines.append(ori.include_table_correction_angles(
                    orient_result_dir, orientconf.ccmin))
                extended_lines.append(ori.add_single_station_results(
                    orient_result_dir, orientconf.ccmin))
                extended_lines.append(ori.add_all_stats_figures_baz(
                    orient_result_dir, orientconf.ccmin))
                extended_lines.append(ori.add_all_stats_figures_timing(
                    orient_result_dir, orientconf.ccmin))
                extended_lines.append(ori.add_all_stats_figures_ccs(
                    orient_result_dir, orientconf.ccmin))
                ...
                for comp in ['Z', 'R', 'T']:
                    extended_lines.append(gains.add_gain_map(
                                          comp, gain_result_dir))
                    extended_lines.append(gains.add_gain_median_mean(
                                          comp, gainconf.gain_factor_method,
                                          gain_result_dir))
                    extended_lines.append(gains.add_gain_allevents_plot(
                                          comp, gain_result_dir))
                    extended_lines.append(gains.add_single_station_results(
                                          comp, gain_result_dir))
                ...
                for comp in ['Z', 'R', 'T']:
                    extended_lines.append(psds.add_psds_stats(
                                          comp, psd_result_dir))
                    extended_lines.append(psds.add_psds_flatranges(
                                          comp, psd_result_dir))

                extended_lines.append(psds.include_table_psds_flatranges(
                                      psd_result_dir))
                ...
                extended_lines.append(time.add_timing_results(
                                      time_result_dir))
                extended_lines.append(time.add_timing_array(
                                      time_result_dir))
                extended_lines.append(time.add_timing_perstation(
                                      time_result_dir))
