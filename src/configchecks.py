...
def check_general_settings(gensettings):
    ...
    if os.path.exists(gensettings.work_dir):
        ...
        logger.error("ERROR: Working directory not found: %s." % gensettings.work_dir)
        error = True
...
def check_catalog_settings(catalogconf):
    ...
    if not cf.search_events:
        if not (cf.use_local_catalog or cf.use_local_subsets):
            logger.error(
                " ERROR: Either use option search_events or provide a local catalog (use_local_catalog and catalog_fn) "
        ...
        if cf.use_local_catalog and cf.use_local_subsets:
            logger.error(
                " ERROR: Either use option use_local_catalog  "
                + " or option use_local_subsets."
            )
            error = True
        ...
        if cf.subset_of_local_catalog and cf.use_local_subsets:
            logger.error(
                " ERROR: Either use option use_local_catalog and subset_of_local_catalog  "
                + " or option use_local_subsets."
            )
            error = True
    else:
    ...
    if cf.catalog_fn:
        if os.path.exists(cf.catalog_fn):
            ...
            logger.error(" ERROR: Catalog file not found: %s." % cf.catalog_fn)
            error = True

    # select events
    if cf.min_mag < 6.0:
        logger.warning(
            " Events with smaller magnitudes than Mw 6.0 are unlikely"
            + " to have distinct P and Rayleigh waves needed for the test."
            + " We recommend using Mw 6.5-7.5."
        )
    if cf.max_mag > 8.0:
        logger.warning(
            " Events with larger magnitudes than Mw 8.0 may be very complex"
    ...
    if cf.min_dist_km < 1000:
        logger.info(
            " If possible, the events should be at a distance >> than the inter-station distances."
        )
    ...
    if cf.depth_options["shallow"]:
        logger.debug(
            " Catalog subset depth, shallow events: %s-%s km."
            % (
    ...
    if cf.depth_options["deep"]:
        logger.debug(
            " Catalog subset depth, deep events: %s-%s km."
            % (cf.depth_options["deep"][0] / 1000, cf.depth_options["deep"][1] / 1000)
        )
    ...
    if cf.depth_options["shallow"] and cf.depth_options["deep"]:
        if cf.depth_options["shallow"][1] > cf.depth_options["deep"][1]:
            logger.error(
                " ERROR: depth_options: deep settings shallower than shallow settings."
            )
            error = True

    if cf.wedges_width > 15:
        logger.debug(
            " The wedges width defines in which azimuthal bins events are found."
    ...
    if not cf.mid_point:
        logger.debug(
            " Mid point of station array/network for subsets and of maps for catalog plots determined autmatically."
        )
    else:
        if not len(cf.mid_point) == 2:
            logger.error(
                " Mid point of station array/network for subsets and of maps for catalog plots must have length=2 (lat, lon) or"
    ...
    if cf.dist > 165.0 and (cf.plot_catalog_all or cf.plot_catalog_subset):
        logger.error(
            " Plotting distance dist [deg] for azimuthal plots cannot be larger than 165 degrees"
        )
        error = True

    logger.debug(
        " Plotting distance dist [deg] for azimuthal plots set to %s degrees." % cf.dist
    )
...
def check_metadata_settings(metaDataconf):
    ...
    if not cf.use_downmeta:
        logger.warning(
            " Set MetaDataDownloadConfig use_downmeta to true to use the downloaded metadata."
        )

    if len(cf.channels_download) != 3:
        logger.error(
            " MetaDataDownloadConfig channels_download takes a string with three characters as input. Use * as wildcard, e.g. HH*."
        )
        error = True

    if cf.dt_start > 0.5 or cf.dt_start < 0 or cf.dt_end < 0.5:
        logger.warning(
            "MetaDataDownloadConfig dt_start is in hours before and dt_end in hours after origin time. Make sure time windows are long enough to include body waves, surface waves and sufficient time for filtering etc."
        )
...
def check_restitution_settings(RestDownconf):
    ...
    if not cf.freqlim[0] < cf.freqlim[1] < cf.freqlim[2] < cf.freqlim[3]:
        logging.error(
            " RestDownRotConfig frelim is defining the frequency range (corner frequencies) for which data is restituted. First entry must be smallest, last largest."
        )
    pass
...
def check_synthetics_settings(synthsconf):
    ...
    if cf.make_syn_data and not (cf.engine_path and cf.store_id):
        logger.error(
            " ERROR: SynthDataConfig To compute synthetic data, the engine path amd gfdb must be defined."
        )
        error = True
...
def check_gain_settings(gainfconf, RestDownconf):
    ...
    if len(cf.gain_factor_method) == 1:
        if cf.gain_factor_method not in ["scale_one", "median_all_avail", "syn_comp"]:
            logging.error(
                " Gain factor method not known. Choose between reference_nsl, syn_comp, median_all_avail, scale_one."
            )
            error = True
    ...
    if cf.fband["corner_hp"] > cf.fband["corner_lp"]:
        logging.error(
            " GainfactorsConfig HP corner of BP filter must be smaller than LP corner."
        )
        error = True

    if cf.fband["corner_lp"] > RestDownconf.freqlim[2]:
        logging.error(
            " GainfactorsConfig LP corner of BP filter can be at maximum be same as limit of restitution frequency limit (3rd entry of RestDownRotConfig.freqlim."
        )
        error = True
    ...
    for x in cf.components:
        if x not in ["Z", "N", "E"]:
            logging.error(
                " GainfactorsConfig Components must Z, N and/or E, e.g. [Z] or [Z,N,E]."
            )
            error = True
...
def check_orient_settings(orientconf):
    ...
    if isinstance(cf.bandpass[0], int):
        logging.error(
            " First part of OrientConfig bandpass is steepness of filter (integer)."
        )
        error = True

    if cf.bandpass[1] > cf.bandpass[2]:
        logging.error(
            " OrientConfig bandpass has form [filter_steepness, fmin, fmax]. fmin must be smaller fmax."
        )
        error = True

    if cf.bandpass[2] > 0.06:
        logging.warning(
            " OrientConfig bandpass should be chosen to emphasize surface waves. Usually fmax is best chosen smaller than 0.05."
        )

    if cf.stop_after_ev < 400:
        logging.warning(
            " OrientConfig stop_after_ev must enable a long time window to do the test with deep frequencies without filer effects. Please choose 600 s if possible."
        )

    if cf.ccmin < 0.7:
        logging.warning(
            " OrientConfig Make sure to set a sufficiently large cross-correlation threshold to get meaningful results. Test e.g. using 0.7, 0.75, 0.8 or even higher."
        )
...
def check_timing_settings(timingconf):
    ...
    if isinstance(cf.bandpass[0], int):
        logging.error(
            " First part of TimingConfig bandpass is steepness of filter (integer)."
        )
        error = True

    if cf.bandpass[1] > cf.bandpass[2]:
        logging.error(
            " TimingConfig bandpass has form [filter_steepness, fmin, fmax]. fmin must be smaller fmax."
        )
        error = True
...
def check_config(configfile):
    ...
    if (
        ...
        logs.info("\n")
        logs.error(
            "--> --> One more more errors in config file found. Please check error messages above."
        )
        sys.exit()
