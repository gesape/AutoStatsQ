...
def gmtplot_catalog_azimuthal(catalog, mid_point, dist, outfile, bin_width):
    ...
    gmt.psbasemap(
        R="0/360/-90/0", J="S0/-90/90/6i", B="xa%sf%s" % (bin_width * 2, bin_width)
    )
    gmt.pscoast(
        R="g",
    ...
    gmt.psxy(
        R="0/360/-90/0",
        J="E%s/%s/%s/6i" % (mid_point[1], mid_point[0], dist),
        in_columns=([ev.lon for ev in catalog], [ev.lat for ev in catalog]),
        G="red",
        S="a12p",
    )
    gmt.save(outfile)
...
def plot_catalog_hist(
    ...
    ax1.set_xlabel(str(bin_width) + r"$^\circ$" + " backazimuth bins", fontsize=10)
    ...
    if full_cat is True:
        plt.savefig(
            os.path.join(data_dir, "results/catalog/cat_hist_magn_dist_%s.png" % d)
        )
    else:
        plt.savefig(
            os.path.join(
                data_dir, "results/catalog/cat_hist_magn_dist_%s_subset.png" % d
            )
        )
    plt.close()
...
def plot_distmagn(dist_array, catalog, data_dir, d, full_cat):
    ...
    if full_cat is True:
        plt.savefig(
            os.path.join(data_dir, "results/catalog/cat_dist_vs_magn_%s.png" % d)
        )
    else:
        plt.savefig(
            os.path.join(data_dir, "results/catalog/cat_dist_vs_magn_%s_subset.png" % d)
        )
    plt.close()
