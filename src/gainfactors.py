    ...
    def finish(self, method, fband, taper, ev_counter):
        ...
        if len(method) == 2:
            ...
            for nslc_id, maxA in self.max_tr.items():
                try:
                    # print(nslc_id[0:2])
                    self.relative_scalings[nslc_id] = (
                        maxA / self.max_tr_syn[nslc_id[0:2]]
                    )
                except:
                    self.logs.warning(
                        " Synthetic data for comparison missing: %s" % nslc_id[0:2]
                    )
...
class AutoGain:
        ...
        for i_ev, event in enumerate(self.events):
            ...
            for i_s, s in enumerate(self.stations):
                ...
                if num.isnan(arrival):
                    skipped += 1
                    self.logs.warning(
                        "skipped %s.%s %s" % (s.network, s.station, event.time)
                    )
                    continue
                else:
                    unskipped += 1
                ...
                tr_generator = self.data_pile.chopper(
                    tmin=arrival - twd[0],
                    tmax=arrival + twd[1],
                ...
                for tr in tr_generator:
                    if not len(tr) > 1 and tr:
                        ...
                        if len(tr.ydata) > 0 and num.max(num.abs(tr.get_ydata())) != 0:
                            ...
                            snr = num.mean(
                                [y * y for y in tr.ydata[st_s : st_s + 60]]
                            ...
                            if debug is True:
                                ...
                                self.logs.debug(
                                    "arrival time %s" % util.time_to_str(arrival)
                                )
                                trace.snuffle(
                                    tr,
                                    markers=[
                        ...
                        for t in tr:
                            ...
                            if (
                                len(tt.ydata) > 0
                                and num.max(num.abs(tt.get_ydata())) != 0
                            ):
                                ...
                                snr = num.mean(
                                    [y * y for y in tt.ydata[st_s : st_s + 60]]
                ...
                if self.method == "syn_comp":
                    for tr in tr_syn_generator:
                        if not len(tr) > 1 and tr:
                            ...
                            if (
                                len(tr.ydata) > 0
                                and num.max(num.abs(tr.get_ydata())) != 0
                            ):
                                dtype = type(tr.ydata[0])
                                tr.ydata -= dtype(tr.get_ydata().mean())
                                st_s = num.argmax(num.abs(tr.ydata)) - 10
                                ...
                                if num.max(num.abs(tr.get_ydata())) != 0:
                                    section.max_tr_syn[tr.nslc_id[0:2]] = num.max(
                                        num.abs(tr.get_ydata())
                                    )
                            ...
                            for t in tr:
                                ...
                                if (
                                    ...
                                    if num.max(num.abs(tt.get_ydata())) != 0:
                                        section.max_tr_syn[tt.nslc_id[0:2]] = num.max(
                                            num.abs(tt.get_ydata())
                                        )
    ...
    def handle_median_stats_option(self):
        ...
        candidates_ref_values = [results_all[0, i_st] for i_st in ind_most_events]
    ...
    def mean(self):
        ...
        if self._mean is None and self.method != "syn":
            self._mean = dict(
                zip(
                    map(lambda x: ".".join(x), self.all_nslc_ids),
        ...
        if self._mean is None and self.method == "syn":
            nslcs = [
                st.split(" ")[1] + "." + st.split(" ")[2] + ".." + "HHZ"
                for st in self.stations
            ]
            self._mean = dict(zip(nslcs, num.nanmean(self.results, axis=0)))
        return self._mean
    ...
    def median(self):
        ...
        if self._median is None and self.method == "syn":
            ...
            self._median = dict(zip(nslcs, num.nanmedian(self.results, axis=0)))
        return self._median
    ...
    def n_ev(self):
        ...
        if self._n_ev is None and self.method != "syn":
            ...
            self._n_ev = dict(
                zip(map(lambda x: ".".join(x), self.all_nslc_ids), nev_used)
            )
        return self._n_ev
    ...
    def save_single_events(self, fn, directory, plot=False):
        ...
        with open(os.path.join(directory, fn), "w") as outfile:
            ...
            for i_line, line in enumerate(results_all):
                if not self.method == "syn":
                    outfile.write(util.time_to_str(self.sections[i_line].event.time))
                else:
